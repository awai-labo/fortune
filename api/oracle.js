...
渡された文章は、オラクルカードのリーディング本文です。すでに完成しています。
...
　かもしれません ／ かもしれない
　と思います ／ と思えます ／ ように思えます
　ように見えます ／ ように読めます ／ と読みたくなります
　でしょう ／ はずです ／ はずで
　たぶん ／ おそらく ／ きっと ／ もしかすると

ここに挙がっていない言い回しでも、断定を避けるために使われているものは、すべてこの枠に数えてください。

本文全体で、この枠の合計を五回以下にしてください。
六回以上ある場合は、多すぎる分を削ります。削り方は次の通りです。

　・同じ語が繰り返されているなら、二回目以降を別の形に言い換えるのではなく、まず削る
　・削った結果、相手の過去を断定する文になってしまう箇所は削らない。そこは残す
　・残す五回は、すべて違う形にする。同じ語を二度使わない
　・同じ語尾が二文続けて並んでいたら、必ず片方を変える

削るときは、推量の言葉だけを取り除き、文の残りはそのまま使ってください。
新しい内容を足さないこと。場面の描写を勝手に増やさないこと。

二　長い文を割る
六十字を超える文を探し、二文に割ってください。
...
直す必要がなければ、渡された文章をそのまま返してください。
直した箇所の説明、前置き、あいさつは一切書かないこと。校正後の本文だけを返してください。`;
        ...
        try {
          const rP = await fetch('https://api.anthropic.com/v1/messages', {
            ...
            body: JSON.stringify({
              ...
              max_tokens: 3000,
              system: systemPolish,
              messages: [{ role: 'user', content: draftT5 }],
            }),
          });
          const dP = await rP.json();
          const polished = dP.content?.find(b => b.type === 'text')?.text || '';
